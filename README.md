...
| `install.ps1` | copies the stubs into a repo, fills the ci inputs, warns if the repo has no `AGENTS.md` |
...
1. Fill `.github/review-context.md`: one paragraph of what the repo is, its stack, and the contracts a reviewer must hold the diff against. Claude reads it from the **base** branch, so a PR cannot rewrite its own reviewer's instructions. **Codex does not read it in cloud mode** — only `mode: action` does — it reads the repo-root `AGENTS.md`, under 2 KB. A repo with no `AGENTS.md` gets a `CODEX-REVIEW: PASS` that means "Codex found nothing", not "Codex checked our rules" (verified 2026-09-17 on `camedu-io/district-intel` and `crm`, which had neither), so `install.ps1` warns when it is absent. It does not write one: `AGENTS.md` is the repo's own file, describing rules that exist whether or not this CI is installed, and in many repos `CLAUDE.md` includes it. A CI installer authoring it would push review wording into every agent session that reads it.
...
- [2026-09-22] gosuperlab/website PR #119 (chore/, install itself) + PRs #120–#123 (wo/*, solo-harness) | install green first try with `setup: node-pnpm` + a new `scripts/verify.ps1` (pnpm verify + Playwright e2e under MOCK_EXTERNALS=1, 7m51s); the workflow-changing PR was correctly skipped by claude-review and hand-merged. Over the next 6 hours the harness shipped four wo/* PRs: `claude / claude-review` produced 9 CHANGES REQUESTED verdicts (all real: unscoped CHECK constraint, stuck `running` jobs, unchecked write errors, photo import wiping on re-claim, docs contradicting the repo) before each PASS, and the gate correctly left every wo/* PR to ship.ps1 | Lesson: the repo predated the template and had never been in the 2026-09-17 install batch; ship.ps1 needed two fixes to live with the template: read the verdict from the `AUTO-REVIEW:` PR review (not a comment), and wait for GitHub to register the pushed head before `gh pr checks` (it returned the previous head's red review for ~90 s). Codex review never fired: wo/* and chore/ are outside its branch types, so the second-reviewer path is still unexercised on this repo.
